...
fetch("http://localhost:3000/pups")
  .then(r => r.json())
  .then(pupsArray => {
    pups = pupsArray // fills the data of the empty array we created called pups on top
...
function renderPups(pupsArray) {
  ...
  pupsArray.forEach(pup => {
    ...
    pupsSpan.addEventListener('click', () => { // ADDEVENTLISTENER GOES TO THE ELEMENT TAG OF THE OBJECT, NOT THE ACTUAL OBJECT ITSELF. DO NOT WRITE pup.addEventListener
      ...
      `
      const button = dogInfo.querySelector("button") // need to select this button for if statement down below. since there's only one button inside dogInfo container, querySelector inside dogInfo and just type button

      if (pup.isGoodDog === true) { // changes the text inside the button defined above according to boolean of isGoodDog attribute
      ...
      button.addEventListener("click", () => { 
        pup.isGoodDog = !pup.isGoodDog // switches boolean value of isGoodDog attribute
        const updatedDogInfo = { isGoodDog: pup.isGoodDog } // ask how this grabs the value of isGoodDog

        // ...do (a patch) fetch
        fetch(`http://localhost:3000/pups/${pup.id}`, { // need to get specific dog id since this is a patch
          method: "PATCH",
          headers: {
